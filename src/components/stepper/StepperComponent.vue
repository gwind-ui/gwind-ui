...
import { Stepper, StepperItem, StepperSeparator, StepperTrigger, StepperTitle, StepperDescription, StepperIndicator } from "./"
...
<template>
    <Stepper class="mx-auto flex flex-col w-full max-w-md justify-start gap-10">
        <StepperItem v-for="step in steps" :key="step.step" v-slot="{ state }" class="relative flex w-full mobile:items-center md:items-start lg:items-start gap-4" :step="step.step">
            <StepperSeparator v-if="step.step !== steps[steps.length - 1].step" class="absolute left-[19px] top-[39px] block h-[105%] w-0.5 shrink-0 rounded-full bg-muted group-data-[state=completed]:bg-primary" />
